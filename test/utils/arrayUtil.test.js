...
describe('#ArrayUtil', () => {
    describe('#insert', () => {
        ...
        it('insert middle', () => {
            ...
            assert.deepStrictEqual(testArray, [2, 4, 5, 6, 8]);
        })

        it('insert when index === 0', () => {
            insert(testArray, 0, 1);
            assert.deepStrictEqual(testArray, [1, 2, 4, 6, 8]);
        })

        it('insert when index === testArray.length - 1', () => {
            insert(testArray, testArray.length - 1, 7);
            assert.deepStrictEqual(testArray, [2, 4, 6, 7, 8]);
        })
        ...
        it('insert when index === testArray.length', () => {
            ...
            assert.deepStrictEqual(testArray, [2, 4, 6, 8, 9]);
        })
    })
})